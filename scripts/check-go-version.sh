...
version_regex="go-version: [0-9]\.[0-9]\+"
...
go_mod_path="./go.mod"
go_mod_version=$(grep -oP 'go\s+\K[0-9]\.[0-9]+' $go_mod_path)
...
done < <(find . -type f -name "*.yml" -exec grep -o -H "$version_regex" {} +)

# Extract go version from Alpine Image makefile
alpine_makefile_path="./docker/Makefile"
go_alpine_version=$(grep -oP 'golang:[0-9]\.[0-9]+' "$alpine_makefile_path")
