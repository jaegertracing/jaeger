...
func (p *Annotation) Validate() error {
	return nil
}
...
func (p *BinaryAnnotation) Validate() error {
	return nil
}
...
func (p *zipkinCollectorProcessorSubmitZipkinBatch) Process(ctx context.Context, seqId int32, iprot, oprot thrift.TProtocol) (success bool, err thrift.TException) {
	...
	if retval, err2 := p.handler.SubmitZipkinBatch(ctx, args.Spans); err2 != nil {
		...
		_exc10 := thrift.NewTApplicationException(thrift.INTERNAL_ERROR, "Internal error processing submitZipkinBatch: "+err2.Error())
		if err2 := oprot.WriteMessageBegin(ctx, "submitZipkinBatch", thrift.EXCEPTION, seqId); err2 != nil {
...
func (p *ZipkinCollectorSubmitZipkinBatchArgs) ReadField1(ctx context.Context, iprot thrift.TProtocol) error {
	...
	for i := 0; i < size; i++ {
		...
		p.Spans = append(p.Spans, _elem11)
	}
	if err := iprot.ReadListEnd(ctx); err != nil {
		return thrift.PrependError("error reading list end: ", err)
	}
