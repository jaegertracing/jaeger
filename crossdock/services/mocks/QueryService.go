...
import (
	"github.com/jaegertracing/jaeger/internal/converter/json"
...
func (_m *QueryService) GetTraces(serviceName string, operation string, tags map[string]string) ([]*json.Trace, error) {
	...
	var r0 []*json.Trace
	var r1 error
	if rf, ok := ret.Get(0).(func(string, string, map[string]string) ([]*json.Trace, error)); ok {
		return rf(serviceName, operation, tags)
	}
	if rf, ok := ret.Get(0).(func(string, string, map[string]string) []*json.Trace); ok {
		r0 = rf(serviceName, operation, tags)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]*json.Trace)
		}
	}
