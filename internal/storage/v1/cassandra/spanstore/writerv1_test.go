...
func TestWriteSpan(t *testing.T) {
	...
	insertions := []dbmodel.TagInsertion{
		{
			ServiceName: "service-a",
			TagKey:      "x",
			TagValue:    "y",
	...
	mockWriter.On("WriteSpan", dbmodel.FromDomain(span), insertions).Return(nil)
	opts := applyOptions()
	writer := SpanWriterV1{spanWriter: mockWriter, tagFilter: opts.tagFilter}
...
func TestSpanWriterV1Close(t *testing.T) {
	...
	writer := SpanWriterV1{spanWriter: mockWriter}
