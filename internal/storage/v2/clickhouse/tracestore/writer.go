		...
		for _, ss := range rs.ScopeSpans().All() {
			for _, span := range ss.Spans().All() {
				...
				err = batch.Append(
					...
					sr.EventNames,
					sr.EventTimestamps,
					toTuple(sr.EventAttributes.Keys, sr.EventAttributes.Values, sr.EventAttributes.Types),
					sr.LinkTraceIDs,
					sr.LinkSpanIDs,
					sr.LinkTraceStates,
					toTuple(sr.LinkAttributes.Keys, sr.LinkAttributes.Values, sr.LinkAttributes.Types),
					sr.ServiceName,
...
func toTuple[T any](keys [][]string, values [][]T, types [][]string) [][][]any {
	tuple := make([][][]any, 0, len(keys))
	for i := range keys {
		inner := make([][]any, 0, len(keys[i]))
		for j := range keys[i] {
			inner = append(inner, []any{keys[i][j], values[i][j], types[i][j]})
		}
