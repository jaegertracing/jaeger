...
// see ServesAnyFilter, which is what callers ask rather than comparing against nil.
...
// ServesAnyFilter reports whether the Reader declared support for any part of a structured
// filter. A nil FilterCapabilities and an empty one both answer false, so a caller never has
// to tell "declared nothing" from "did not declare", and a Reader cannot half-opt-in.
func (c *FilterCapabilities) ServesAnyFilter() bool {
