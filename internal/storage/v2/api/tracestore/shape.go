...
// able to invert it. A query that already carries a Filter is returned as it is, and one with
// no predicates at all becomes an empty conjunction rather than a nil filter, so a caller
// never has to distinguish "no predicates" from "not converted".
func (q TraceQueryParams) ToFilterShape() TraceQueryParams {
	...
	converted.Filter = &expression.Call{Op: expression.OpAnd, Args: predicates}
	return converted
}
...
func durationBound(op expression.Operator, d time.Duration) expression.Expression {
