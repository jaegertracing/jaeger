...
func TestTimeRangeIndices(t *testing.T) {
	...
	testCases := []struct {
	...
	}{
		{
			...
			endTime:   today,
			expected: []string{
				IndexWithDate(config.SpanIndexName, dateLayout, today),
			},
		},
		{
			...
			expected: []string{
				IndexWithDate(config.SpanIndexName, dateLayout, today),
				IndexWithDate(config.SpanIndexName, dateLayout, yesterday),
			},
		},
		{
			...
			expected: []string{
				IndexWithDate(config.SpanIndexName, dateLayout, today),
				IndexWithDate(config.SpanIndexName, dateLayout, yesterday),
				IndexWithDate(config.SpanIndexName, dateLayout, twoDaysAgo),
			},
		},
	}
	for _, testCase := range testCases {
		actual := timeRangeIndices(config.SpanIndexName, dateLayout, testCase.startTime, testCase.endTime, -24*time.Hour)
		assert.Equal(t, testCase.expected, actual)
	}
}

func TestIndexWithDate(t *testing.T) {
	actual := IndexWithDate(config.SpanIndexName, "2006-01-02", time.Date(1995, time.April, 21, 4, 21, 19, 95, time.UTC))
...
func TestLoggingTimeRangeIndexFn_NoDebug(t *testing.T) {
	...
	result := wrapped(config.SpanIndexName, "2006-01-02", start, end, -24*time.Hour)
...
func TestTimeRangeIndicesFn_ReadWriteAliases(t *testing.T) {
	...
	result := fn(config.SpanIndexName, "2006-01-02", time.Now(), time.Now(), -24*time.Hour)
...
func TestAddRemoteReadClusters(t *testing.T) {
	...
	result := fn(config.SpanIndexName, "2006-01-02", start, end, -24*time.Hour)
