...
type Indices struct {
    // SpanAlias, when set, forces Jaeger to write/read spans through the given
    // index or alias name and bypasses any prefix- or date-based pattern.
    SpanAlias string `mapstructure:"span_alias"`
    // ServiceAlias behaves the same but for service indices.
