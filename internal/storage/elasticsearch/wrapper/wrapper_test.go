...
const componentTemplateBody = `{"template":{"settings":{}}}`

func TestClientWrapper_CreateComponentTemplate(t *testing.T) {
	for _, version := range []es.BackendVersion{es.ElasticV7, es.ElasticV8} {
		t.Run(version.String(), func(t *testing.T) {
			...
			require.NoError(t, c.CreateComponentTemplate(context.Background(), "jaeger.spans@mappings", componentTemplateBody))
			assert.Equal(t, http.MethodPut, gotMethod)
			assert.Equal(t, "/_component_template/jaeger.spans@mappings", gotPath)
			assert.JSONEq(t, componentTemplateBody, gotBody)
		})
	}
}

func TestClientWrapper_CreateComponentTemplate_AcceptsCreated(t *testing.T) {
	...
	require.NoError(t, c.CreateComponentTemplate(context.Background(), "jaeger.spans@mappings", "{}"))
}

func TestClientWrapper_CreateComponentTemplate_Failures(t *testing.T) {
	for _, version := range []es.BackendVersion{es.ElasticV7, es.ElasticV8} {
		t.Run(version.String(), func(t *testing.T) {
