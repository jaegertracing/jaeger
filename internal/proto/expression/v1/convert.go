...
// ToFilter converts a filter received over the wire into the filter AST, and refuses one that
// is not well formed — a structure the AST has no place for, or a tree that
// expression.ValidateFilter rejects. Decoding and validating are one step because every wire is
// a trust boundary, and a caller who was handed a filter without being told to validate it
// would pass a malformed tree to a backend, which typically answers such a predicate by
// matching nothing rather than by refusing.
//
// It says nothing about what a backend can *serve*; that is what a backend's declared
// capabilities are for.
func ToFilter(filter *Call) (*expression.Call, error) {
	call, err := toCall(filter)
	if err != nil {
		return nil, err
	}
	if call == nil {
...
func toCall(filter *Call) (*expression.Call, error) {
	if filter == nil {
		return nil, nil
	}
