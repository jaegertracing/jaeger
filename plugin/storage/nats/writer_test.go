...
import (
	...
	"github.com/jaegertracing/jaeger/pkg/nats/mocks"
...
// Checks that nats SpanWriter conforms to spanstore.Writer API
...
func TestnatsWriter(t *testing.T) {
	withSpanWriter(t, func(span *model.Span, w *spanWriterTest) {
		...
		for i := 0; i < 100; i++ {
			...
			if counters["nats_spans_written|status=success"] > 0 {
				break
			}
		}
		w.writer.Close()
...
func TestnatsWriterErr(t *testing.T) {
	withSpanWriter(t, func(span *model.Span, w *spanWriterTest) {
		...
		for i := 0; i < 100; i++ {
			time.Sleep(time.Microsecond)
			counters, _ := w.metricsFactory.Snapshot()
			if counters["nats_spans_written|status=failure"] > 0 {
				break
			}
		}
		w.writer.Close()
