...
type APIHandler struct {
	proxyService *proxysvc.ProxyService
...
func NewAPIHandler(proxyService *proxysvc.ProxyService, options ...HandlerOption) *APIHandler {
	aH := &APIHandler{
...
func (aH *APIHandler) getServices(w http.ResponseWriter, r *http.Request) {
	services, err := aH.proxyService.GetServices(r.Context())
...
func (aH *APIHandler) getOperationsLegacy(w http.ResponseWriter, r *http.Request) {
	...
	operations, err := aH.proxyService.GetOperations(r.Context(),
...
func (aH *APIHandler) getOperations(w http.ResponseWriter, r *http.Request) {
	...
	operations, err := aH.proxyService.GetOperations(
		r.Context(),
...
func (aH *APIHandler) search(w http.ResponseWriter, r *http.Request) {
	...
	if len(tQuery.traceIDs) > 0 {
		...
		tracesFromStorage, err = aH.proxyService.FindTraces(r.Context(), &tQuery.TraceQueryParameters)
		if aH.handleError(w, err, http.StatusInternalServerError) {
			return
		}
	}
...
func (aH *APIHandler) tracesByIDs(ctx context.Context, traceIDs []model.TraceID) ([]*model.Trace, []structuredError, error) {
	...
	for _, traceID := range traceIDs {
		if trace, err := aH.proxyService.GetTrace(ctx, traceID); err != nil {
...
func (aH *APIHandler) dependencies(w http.ResponseWriter, r *http.Request) {
	...
	dependencies, err := aH.proxyService.GetDependencies(r.Context(), endTs, lookback)
	if aH.handleError(w, err, http.StatusInternalServerError) {
		return
	}
...
func (aH *APIHandler) convertModelToUI(trace *model.Trace, adjust bool) (*ui.Trace, *structuredError) {
	...
	if adjust {
		var err error
		trace, err = aH.proxyService.Adjust(trace)
		if err != nil {
...
func (aH *APIHandler) getTrace(w http.ResponseWriter, r *http.Request) {
	...
	trace, err := aH.proxyService.GetTrace(r.Context(), traceID)
	if err == spanstore.ErrTraceNotFound {
...
// It passes the traceID to proxyService.ArchiveTrace for writing.
func (aH *APIHandler) archiveTrace(w http.ResponseWriter, r *http.Request) {
	...
	err := aH.proxyService.ArchiveTrace(r.Context(), traceID)
	if err == spanstore.ErrTraceNotFound {
		aH.handleError(w, err, http.StatusNotFound)
		return
	}
