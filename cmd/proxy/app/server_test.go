...
func TestCreateTLSServerSinglePortError(t *testing.T) {
	...
	_, err := NewServer(zap.NewNop(), &proxysvc.ProxyService{},
		&ProxyOptions{HTTPHostPort: ":8080", GRPCHostPort: ":8080", TLSGRPC: tlsCfg, TLSHTTP: tlsCfg}, opentracing.NoopTracer{})
	assert.NotNil(t, err)
}
...
func TestCreateTLSGrpcServerError(t *testing.T) {
	...
	_, err := NewServer(zap.NewNop(), &proxysvc.ProxyService{},
		&ProxyOptions{HTTPHostPort: ":8080", GRPCHostPort: ":8081", TLSGRPC: tlsCfg}, opentracing.NoopTracer{})
	assert.NotNil(t, err)
}
...
func TestCreateTLSHttpServerError(t *testing.T) {
	...
	_, err := NewServer(zap.NewNop(), &proxysvc.ProxyService{},
		&ProxyOptions{HTTPHostPort: ":8080", GRPCHostPort: ":8081", TLSHTTP: tlsCfg}, opentracing.NoopTracer{})
	assert.NotNil(t, err)
}
...
func TestServerHTTPTLS(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			proxysvc := proxysvc.NewProxyService(proxysvc.ProxyServiceOptions{}, zap.NewNop())
			server, err := NewServer(flagsSvc.Logger, proxysvc,
				serverOptions,
...
func TestServerBadHostPort(t *testing.T) {
	_, err := NewServer(zap.NewNop(), &proxysvc.ProxyService{},
		&ProxyOptions{HTTPHostPort: "8080", GRPCHostPort: "127.0.0.1:8081", BearerTokenPropagation: true},
		opentracing.NoopTracer{})

	assert.NotNil(t, err)
	_, err = NewServer(zap.NewNop(), &proxysvc.ProxyService{},
		&ProxyOptions{HTTPHostPort: "127.0.0.1:8081", GRPCHostPort: "9123", BearerTokenPropagation: true},
		opentracing.NoopTracer{})
...
func TestServerSinglePort(t *testing.T) {
	...
	server, err := NewServer(flagsSvc.Logger, proxysvc,
		&ProxyOptions{GRPCHostPort: hostPort, HTTPHostPort: hostPort, BearerTokenPropagation: true},
		opentracing.NoopTracer{})
	assert.Nil(t, err)
	assert.NoError(t, server.Start())
...
func TestServerGracefulExit(t *testing.T) {
	...
	proxysvc := &proxysvc.ProxyService{}
	tracer := opentracing.NoopTracer{}
	server, err := NewServer(flagsSvc.Logger, proxysvc, &ProxyOptions{GRPCHostPort: hostPort, HTTPHostPort: hostPort}, tracer)
	assert.Nil(t, err)
	assert.NoError(t, server.Start())
	go func() {
		for s := range server.HealthCheckStatus() {
...
func TestServerHandlesPortZero(t *testing.T) {
	...
	proxysvc := &proxysvc.ProxyService{}
	tracer := opentracing.NoopTracer{}
	server, err := NewServer(flagsSvc.Logger, proxysvc, &ProxyOptions{GRPCHostPort: ":0", HTTPHostPort: ":0"}, tracer)
	assert.Nil(t, err)
	assert.NoError(t, server.Start())
	server.Close()

	message := logs.FilterMessage("Proxy server started")
