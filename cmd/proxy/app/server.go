...
func NewServer(logger *zap.Logger, proxysvc *proxysvc.ProxyService, options *ProxyOptions, tracer opentracing.Tracer) (*Server, error) {
	...
	grpcServer, err := createGRPCServer(proxysvc, options, logger, tracer)
	if err != nil {
		return nil, err
	}

	httpServer, err := createHTTPServer(proxysvc, options, tracer, logger)
	if err != nil {
		return nil, err
	}
...
func createGRPCServer(proxysvc *proxysvc.ProxyService, options *ProxyOptions, logger *zap.Logger, tracer opentracing.Tracer) (*grpc.Server, error) {
	...
	handler := NewGRPCHandler(proxysvc, logger, tracer)
	api_v2.RegisterQueryServiceServer(server, handler)
	return server, nil
}

func createHTTPServer(proxysvc *proxysvc.ProxyService, queryOpts *ProxyOptions, tracer opentracing.Tracer, logger *zap.Logger) (*http.Server, error) {
