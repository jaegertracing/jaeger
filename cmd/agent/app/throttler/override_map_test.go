...
func TestOverrideMap(t *testing.T) {
	...
	assert.True(t, o.Set("a", 1.0))
	assert.True(t, o.Set("b", 2.0))
	assert.False(t, o.Set("c", 3.0))
	assert.Equal(t, 1.0, o.Get("a").(float64))
	assert.Equal(t, 2.0, o.Get("b").(float64))
	assert.Equal(t, 0.0, o.Get("c").(float64))

	o.SetDefault(3.0)
	assert.Equal(t, 3.0, o.Get("c").(float64))

	assert.True(t, o.IsFull())
	assert.True(t, o.Has("a"))
	o.Delete("a")
	assert.Equal(t, 3.0, o.Get("a").(float64))
	assert.False(t, o.IsFull())
	assert.False(t, o.Has("a"))

	assert.True(t, o.Set("c", 4.0))
	assert.True(t, o.IsFull())
	maxOverrides--
	o.UpdateMaxOverrides(maxOverrides)
