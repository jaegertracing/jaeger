...
func (gw *testGateway) runGatewayGetTrace(t *testing.T) {
	...
	gw.verifyTraces(t, "/api/v3/traces/1", traceID)
}
...
func (gw *testGateway) runGatewayFindTraces(t *testing.T) {
	...
	gw.verifyTraces(t, "/api/v3/traces?"+q.Encode(), traceID)
}

func (gw *testGateway) verifyTraces(t *testing.T, url string, expectedTraceID pcommon.TraceID) {
