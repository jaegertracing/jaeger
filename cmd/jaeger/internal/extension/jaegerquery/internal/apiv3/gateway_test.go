...
func runGatewayTests(
	...
	t.Run("DeepDependencies", gw.runGatewayDeepDependencies)
}
...
func (gw *testGateway) runGatewayDeepDependencies(t *testing.T) {
	body, statusCode := gw.execRequest(t, "/api/v3/deep-dependencies")
	require.Equal(t, http.StatusOK, statusCode)
	gw.verifySnapshot(t, body)
}
