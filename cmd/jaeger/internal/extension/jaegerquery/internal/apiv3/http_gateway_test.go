...
func TestDetermineStatusCode(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			err:            spanstore.ErrTraceNotFound,
			expectedStatus: http.StatusNotFound,
		},
		{
			name:           "validation error - search depth",
			err:            errors.New("search depth must be greater than 0 and less than max traces"),
			expectedStatus: http.StatusBadRequest,
		},
		{
			name:           "validation error - max traces",
			err:            errors.New("max traces must be greater than zero"),
			expectedStatus: http.StatusBadRequest,
		},
		{
			name:           "validation error - start time required",
			err:            errors.New("start time is required"),
			expectedStatus: http.StatusBadRequest,
		},
		{
			name:           "validation error - end time",
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			status := determineStatusCode(tc.err)
			assert.Equal(t, tc.expectedStatus, status)
		})
	}
}

func TestHTTPGatewayGetTraceValidationError(t *testing.T) {
	gw := setupHTTPGatewayNoServer(t, "")
	gw.reader.On("GetTraces", matchContext, mock.AnythingOfType("[]tracestore.GetTraceParams")).
		Return(iter.Seq2[[]ptrace.Traces, error](func(yield func([]ptrace.Traces, error) bool) {
			yield(nil, errors.New("search depth must be greater than 0 and less than max traces"))
		})).Once()
	...
	assert.Contains(t, w.Body.String(), "search depth must be greater than 0 and less than max traces")
}

func TestHTTPGatewayFindTracesValidationError(t *testing.T) {
	q, qp := mockFindQueries()
	r, err := http.NewRequest(http.MethodGet, "/api/v3/traces?"+q.Encode(), http.NoBody)
	require.NoError(t, err)
	w := httptest.NewRecorder()
	...
	gw.reader.
		On("FindTraces", matchContext, qp).
		Return(iter.Seq2[[]ptrace.Traces, error](func(yield func([]ptrace.Traces, error) bool) {
			yield(nil, errors.New("start time is required"))
		})).Once()

	gw.router.ServeHTTP(w, r)
	// Validation errors should return 400 Bad Request
	assert.Equal(t, http.StatusBadRequest, w.Code)
	assert.Contains(t, w.Body.String(), "start time is required")
}

func TestHTTPGatewayGetTraceInternalError(t *testing.T) {
	gw := setupHTTPGatewayNoServer(t, "")
	gw.reader.On("GetTraces", matchContext, mock.AnythingOfType("[]tracestore.GetTraceParams")).
		Return(iter.Seq2[[]ptrace.Traces, error](func(yield func([]ptrace.Traces, error) bool) {
