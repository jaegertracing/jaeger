...
func TestGetArchivedTraceSuccess(t *testing.T) {
	archiveData := ptrace.NewTraces()
	rs := archiveData.ResourceSpans().AppendEmpty()
	rs.Resource().Attributes().PutStr("service.name", "archived-service")
	span := rs.ScopeSpans().AppendEmpty().Spans().AppendEmpty()
	span.SetTraceID(otelTraceID)
	...
	mockReader.On("GetTraces",
		mock.Anything,
		mock.Anything,
	).Return(mockGetTracesReturn(archiveData)).Once()
...
func TestArchiveTrace_SuccessWithTimeWindow(t *testing.T) {
	...
	withTestServer(t, func(ts *testServer) {
		...
		ts.spanReader.On("GetTraces", mock.AnythingOfType("*context.valueCtx"), []tracestore.GetTraceParams{expectedQuery}).
			Return(mockGetTracesReturn(otelMockTrace), nil).Once()
		var response structuredTraceResponse
