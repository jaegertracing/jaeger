...
func TestNewHandlerInitialisesStore(t *testing.T) {
	h := NewHandler(zap.NewNop(), "ws://example", "/jaeger", 1<<20, nil)
...
func TestRegisterRoutesMountsChatEndpoint(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			h := NewHandler(zap.NewNop(), "ws://127.0.0.1:1", tc.basePath, 1<<20, nil)
...
func TestNewHandlerNormalizesTrailingSlash(t *testing.T) {
	h := NewHandler(zap.NewNop(), "ws://127.0.0.1:1", "/jaeger/", 1<<20, nil)
