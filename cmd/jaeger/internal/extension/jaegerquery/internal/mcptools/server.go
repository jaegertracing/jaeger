...
func RegisterTools(server *mcp.Server, queryAPI *querysvc.QueryService, cfg Config) {
	...
	}, handlers.NewSearchTracesHandler(queryAPI, cfg.MaxSearchResults))

	mcp.AddTool(server, &mcp.Tool{
		Name: "get_span_details",
		Description: "Fetch full span data (attributes, events, links, status) for specific spans. " +
			"Returns verbose output per span.",
	}, handlers.NewGetSpanDetailsHandler(queryAPI, cfg.MaxSpanDetailsPerRequest))
	...
	}, handlers.NewGetTraceErrorsHandler(queryAPI, cfg.MaxSpanDetailsPerRequest))
	...
	}, handlers.NewGetTraceTopologyHandler(queryAPI, cfg.MaxSpanDetailsPerRequest))
	...
	}, handlers.NewGetCriticalPathHandler(queryAPI))
	...
	}, handlers.NewGetDependenciesHandler(queryAPI))

	// read_skill is backed by the embedded skill playbooks rather than the
	// QueryService. fs.Sub failing would mean the embed itself is malformed
	// (a build-time guarantee), so the error is not actionable at runtime.
	skillsFS, _ := fs.Sub(skillsEmbedFS, "skills")
	mcp.AddTool(server, &mcp.Tool{
		Name: "read_skill",
