...
const testGateway = "# Available Skills\n\n- skill-a — Does A\n- skill-b — Does B\n"

func TestReadSkillHandler_EmptyPathReturnsGateway(t *testing.T) {
	h := &readSkillHandler{skillsFS: testSkillsFS(), maxFileSize: testMaxFileSize, gateway: testGateway}
	...
	assert.Contains(t, tc.Text, "skill-a")
}

func TestReadSkillHandler_ReadSkillMD(t *testing.T) {
	h := &readSkillHandler{skillsFS: testSkillsFS(), maxFileSize: testMaxFileSize, gateway: testGateway}
...
func TestReadSkillHandler_FileNotFound(t *testing.T) {
	h := &readSkillHandler{skillsFS: testSkillsFS(), maxFileSize: testMaxFileSize, gateway: testGateway}
	_, _, err := h.handle(context.Background(), &mcp.CallToolRequest{}, types.ReadSkillInput{Path: "nonexistent.md"})
	require.Error(t, err)
	assert.Contains(t, err.Error(), "cannot stat")
}

func TestNewReadSkillHandler(t *testing.T) {
	handler := NewReadSkillHandler(testSkillsFS(), testMaxFileSize, testGateway)
