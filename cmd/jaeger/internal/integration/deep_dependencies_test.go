...
// Structs below mirror cmd/jaeger/internal/extension/jaegerquery/internal/ddg.
// That package is internal to jaegerquery, so the integration package cannot import it.
type ddgPayloadEntry struct {
	Service   string `json:"service"`
	Operation string `json:"operation"`
}

type ddgAttribute struct {
	Key   string `json:"key"`
	Value string `json:"value"`
}

type ddgPayloadPath struct {
	Path       []ddgPayloadEntry `json:"path"`
	Attributes []ddgAttribute    `json:"attributes"`
}

type ddgPayload struct {
	Dependencies []ddgPayloadPath `json:"dependencies"`
}
...
func TestDeepDependenciesEndpoint(t *testing.T) {
	...
	t.Run("no_service_param_uses_default", func(t *testing.T) {
		...
		const defaultFocalService = "customer"
		found := false
...
// query parameter and returns the parsed payload.
func queryDeepDependencies(t *testing.T, baseURL, service string) ddgPayload {
	t.Helper()
	...
	require.Equal(t, http.StatusOK, resp.StatusCode, "expected HTTP 200 from %s", urlString)
	assert.Equal(t, "application/json", resp.Header.Get("Content-Type"))

	body, err := io.ReadAll(resp.Body)
	require.NoError(t, err)

	var payload ddgPayload
	require.NoError(t, json.Unmarshal(body, &payload), "response body should be valid JSON: %s", string(body))
