...
The connector runs the storage write inside the same `exporterhelper` pipeline as the exporter, so `queue` (including `wait_for_result` and `batch`) and `retry_on_failure` are configured the same way and mean the same thing. Its outcome per batch:
