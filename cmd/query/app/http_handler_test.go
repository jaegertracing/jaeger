...
func TestGetTraceWithTimeWindowSuccess(t *testing.T) {
	...
	ts.spanReader.On("GetTrace", mock.AnythingOfType("*context.valueCtx"), spanstore.TraceGetParameters{
...
func TestGetTrace(t *testing.T) {
	...
	for _, tc := range testCases {
		...
		t.Run(testCase.suffix, func(t *testing.T) {
			...
			ts.spanReader.On("GetTrace", mock.AnythingOfType("*context.valueCtx"), spanstore.TraceGetParameters{TraceID: model.NewTraceID(0, 0x123456abc)}).
...
func TestSearchByTraceIDSuccessWithArchive(t *testing.T) {
	...
	archiveReadMock.On("GetTrace", mock.AnythingOfType("*context.valueCtx"), mock.AnythingOfType("spanstore.TraceGetParameters")).
		Return(mockTrace, nil).Twice()
...
func TestSearchTenancyFlowTenantHTTP(t *testing.T) {
	...
	ts.spanReader.On("GetTrace", mock.MatchedBy(func(v any) bool {
		...
		return true
	}), mock.AnythingOfType("spanstore.TraceGetParameters")).Return(mockTrace, nil).Twice()
	ts.spanReader.On("GetTrace", mock.MatchedBy(func(v any) bool {
		ctx, ok := v.(context.Context)
		...
		return true
	}), mock.AnythingOfType("spanstore.TraceGetParameters")).Return(nil, errStorage).Once()
