...
func TestRegisterStaticHandlerWithUIBasePath(t *testing.T) {
	logger, logBuf := testutils.NewLogger()
	r := mux.NewRouter()

	// Set up UIBasePath to "/ui"
	uiBasePath := "/ui"
	RegisterStaticHandler(r, logger, &QueryOptions{
		QueryOptionsBase: QueryOptionsBase{
			StaticAssets: QueryOptionsStaticAssets{
			...
			UIBasePath: uiBasePath, // Set UIBasePath to "/ui"
		},
	},
		querysvc.StorageCapabilities{ArchiveStorage: true},
	)

	// Test that assets are served with the correct base path
	server := httptest.NewServer(r)
	defer server.Close()

	resp, err := http.Get(server.URL + "/jaeger" + uiBasePath + "/static/asset.txt")
	require.NoError(t, err)
	defer resp.Body.Close()

	body, err := io.ReadAll(resp.Body)
	require.NoError(t, err)
	require.Equal(t, "some asset\n", string(body))

	// Test that UI base path is set correctly in index.html
	resp, err = http.Get(server.URL + "/jaeger" + uiBasePath)
	require.NoError(t, err)
	defer resp.Body.Close()

	body, err = io.ReadAll(resp.Body)
	require.NoError(t, err)
	require.Contains(t, string(body), fmt.Sprintf(`<base href="%s"`, uiBasePath))

	// Test that UI base path is set correctly in index.html file
	indexHTML := string(body)
	require.Contains(t, indexHTML, fmt.Sprintf(`<base href="%s"`, uiBasePath))

	// Verify log messages
	logger.Sync()
	assert.Contains(t, logBuf.String(), "serving static asset", "log message not found")
	assert.Contains(t, logBuf.String(), fmt.Sprintf("/jaeger%s/static/asset.txt", uiBasePath), "log message not found")
}

func TestRegisterStaticHandlerWithoutUIBasePath(t *testing.T) {
	logger, logBuf := testutils.NewLogger()
	r := mux.NewRouter()

	// Do not set UIBasePath
	RegisterStaticHandler(r, logger, &QueryOptions{
		QueryOptionsBase: QueryOptionsBase{
			StaticAssets: QueryOptionsStaticAssets{
				Path:      "fixture",
	...
	resp, err := http.Get(server.URL + "/jaeger/static/asset.txt")
	require.NoError(t, err)
	defer resp.Body.Close()
	...
	// Test that UI base path is not set in index.html
	resp, err = http.Get(server.URL + "/jaeger")
	require.NoError(t, err)
	defer resp.Body.Close()

	body, err = io.ReadAll(resp.Body)
	require.NoError(t, err)
	require.NotContains(t, string(body), "<base href=\"/ui\"")

	// Verify log messages
	logger.Sync()
	assert.Contains(t, logBuf.String(), "serving static asset", "log message not found")
	assert.Contains(t, logBuf.String(), "/jaeger/static/asset.txt", "log message not found")
}
