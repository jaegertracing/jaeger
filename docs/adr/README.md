...
- [ADR-014: Synchronous Elasticsearch/OpenSearch Writes and Lossless Pipelines](014-synchronous-elasticsearch-writes.md) - The `write_mode: sync` writer with idempotent content-hash ids and poison-pill disposition, the `jaeger_storage_writer` dead-letter connector, and the collector pipeline shapes (no `batch` processor, blocking exporter queue) under which a storage error actually reaches the Kafka offset or the OTLP client. Graduated from RFC 0007.
