...
- [ADR-008: ClickHouse Storage Schema](008-clickhouse-storage-schema.md) - Schema design for the native ClickHouse storage backend: typed-attribute representation (`Nested` vs. parallel arrays vs. `Map`), search-optimized primary key, skip indexes, and the materialized views that power `GetTrace`, `GetServices`, `GetOperations`, and attribute-metadata lookups.
