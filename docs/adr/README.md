...
- [ADR-009: UI Base-Path Auto-Detection](009-ui-base-path-auto-detection.md) - Proposal to make the UI self-detect its URL prefix from `window.location`, removing the need to configure `--query.base-path` in the backend for UI delivery and enabling a single Jaeger pod to serve under multiple prefixes.
