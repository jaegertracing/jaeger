...
* **The Kafka receiver** marks a record's offset before processing unless `message_marking.after: true`, in which case it marks only after the pipeline returned. With `on_error: false` (the default) a failed record is not skipped; the receiver holds its offset and pauses the partition until the next consumer-group rebalance, which a single-replica ingester never sees. The receiver has its own `error_backoff`, which rewinds and re-fetches the failed record instead of pausing, but it carries a `max_elapsed_time` after which the partition pauses anyway. The recommended place for the retry is therefore the exporter, with `retry_on_failure.enabled: true` and `max_elapsed_time: 0`: the partition stays live, the offset is held by the retry, and the failed batch is re-sent as is rather than re-fetched.
