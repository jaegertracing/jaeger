...
| M7 | Consolidate the shared and turn-scoped mounts onto **one** `mcp.Server`. The shared mount now reuses the chat gateway's turn-scoped server when both are enabled — the turn-scoped middleware degrades to telemetry-only when no turn is present — so one server serves both; a standalone server is still built for the shared endpoint when there is no chat gateway. | ✅ Done |
| M7.1 | Reap the **shared** endpoint's MCP sessions. `mcptools.WrapHTTP` now returns a closeable `Handler` that owns the session reaping, so every mount gets teardown from construction: the shared mount joins the query server's closer set, and the turn-scoped mount delegates to the same. | ✅ Done |
| M7.2 | ~~Decouple **UI tools** from the presence of the `ai.mcp` config block, so a chat gateway with no `ai.mcp` block still mounts the turn-scoped endpoint to carry per-turn UI tools.~~ **Withdrawn.** After [#9194](https://github.com/jaegertracing/jaeger/pull/9194) the announced base URL (`mcp_base_url`) lives *inside* the `ai.mcp` block, so a chat-only gateway with no `ai.mcp` block has no URL to announce the turn-scoped endpoint — a UI-tools-only server it can never reach is dead code. Per review, if the "AI chat only" scenario is ever needed it would take an explicit flag inside the `ai.mcp` config to turn off the shared telemetry tools while keeping the endpoint. UI-tool dispatch's decoupling is deferred to M6. | ❌ Withdrawn |
