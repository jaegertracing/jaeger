...
| M7 | Consolidate the shared and turn-scoped mounts onto **one** `mcp.Server`. The shared mount now reuses the chat gateway's turn-scoped server when both are enabled — the turn-scoped middleware degrades to telemetry-only when no turn is present — so one server serves both; a standalone server is still built for the shared endpoint when there is no chat gateway. | ✅ Done — [#9024][pr-9024] |
| M7.1 | Reap the **shared** endpoint's MCP sessions. `mcptools.WrapHTTP` now returns a closeable `Handler` that owns the session reaping, so every mount gets teardown from construction: the shared mount joins the query server's closer set, and the turn-scoped mount delegates to the same. | ✅ Done — [#9024][pr-9024] |
| M7.2 | Decouple **UI tools** from `ai.enable_mcp`. The turn-scoped endpoint now mounts for every chat gateway to carry the per-turn UI tools; `enable_mcp` gates only whether the built-in telemetry tools are layered onto it (and the external shared endpoint). So `enable_mcp: false` with chat serves UI tools alone — UI-tool dispatch no longer depends on the telemetry gate, which M6 needs once it drops the ext-method path. | ✅ Done — [#9024][pr-9024] |
