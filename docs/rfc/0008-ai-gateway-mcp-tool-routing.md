...
| M7.2 | Decouple **UI tools** from the presence of the `ai.mcp` config block (the telemetry-MCP toggle, formerly `ai.enable_mcp`). The turn-scoped endpoint now mounts for every chat gateway to carry the per-turn UI tools; the `ai.mcp` block gates only whether the built-in telemetry tools are layered onto it (and the external shared endpoint). So a chat gateway with no `ai.mcp` block serves UI tools alone — UI-tool dispatch no longer depends on the telemetry gate, which M6 needs once it drops the ext-method path. | ✅ Done |
