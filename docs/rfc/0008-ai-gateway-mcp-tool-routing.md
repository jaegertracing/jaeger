...
- **(1) gets worse, not better.** Routing tool calls through a gateway-hosted MCP server over HTTP *adds* a distinct data flow — a second connection back into the process, even on `localhost` — the opposite of minimizing flows. It also adds a *configuration* cost: the sidecar-reachable base URL cannot be inferred and must be supplied via `ai.mcp_base_url` (§4.3). And the extra connection itself remains regardless.
...
**Decision — no default; announce only when `ai.mcp_base_url` is set.** The base URL is the address the *sidecar* uses to dial the gateway back, and the gateway cannot infer it: only the operator knows how their sidecar reaches the query server (directly, through a proxy, across a network namespace, or with TLS terminated elsewhere). So `ai.mcp_base_url` has **no default** — left unset, the HTTP transport is omitted from the announcement, and since HTTP is today's only transport the turn then runs without the turn-scoped tools; set to an absolute URL, that URL is announced ([#9009][pr-9009]).

A `localhost` default (the gateway's own scheme + query port) was considered and rejected. It would announce out of the box for a co-located sidecar, but it bakes in an assumption — that sidecar and gateway share a loopback — which only one topology satisfies; every other deployment would have the gateway announce an address the sidecar cannot reach, and the failure would surface mid-turn when the agent dials it. Announcing nothing until the address is known is safer than announcing a guess: an unset value degrades to a chat turn without tools, a wrong value breaks the turn.

**Reachability probe (M8).** When a base URL *is* configured, the gateway can probe it at startup — analogous to the ACP agent health probe — so a typo or an unbound port surfaces then rather than mid-turn. The probe's reach is limited by design: it runs on the gateway, so it can only confirm the *gateway* reaches the URL, not that the *sidecar* does — a `localhost` value would always pass on the gateway while proving nothing about a remote sidecar. That limit is a further reason the address is operator-supplied rather than defaulted: the configuration, not the probe, is what encodes the reachability the gateway cannot observe for itself.
