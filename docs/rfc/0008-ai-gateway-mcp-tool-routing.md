...
**Decision — infer a localhost default for a co-located sidecar; require config otherwise.** `ai.mcp_base_url` is the address the *sidecar* uses to dial the gateway back. When it is unset, the gateway infers its own localhost address (scheme from its HTTP TLS setting, port from its HTTP config, e.g. `http://localhost:16686`) **if the sidecar is co-located** — that is, `ai.agent_url` is a loopback address. A sidecar the gateway already reaches over loopback shares its network namespace, so it can reach the gateway back the same way; announcing localhost is then safe and needs no configuration, which covers the common single-host deployment. Operators set `ai.mcp_base_url` explicitly only when the sidecar reaches the gateway at a *different* address — behind a proxy, in another network namespace, or with TLS terminated elsewhere. When the sidecar is *not* co-located (`ai.agent_url` is a remote address) and no base URL is set, the gateway cannot infer a reachable address, so it announces nothing rather than a guess that would fail when the agent dials it ([#9009][pr-9009]).

Two poles were rejected. An **unconditional** localhost default announces a broken address to a remote sidecar — one whose `ai.agent_url` is not loopback — and the failure only surfaces when the agent dials it mid-turn. A **no-default / announce-nothing** policy is safe but makes every co-located deployment configure an address the gateway can plainly infer from `ai.agent_url`. Gating the localhost default on co-location keeps the safe half of each: inferred out-of-the-box behavior where loopback reachability is symmetric, and silence where it is not.

**Reachability probe (M8).** The gateway can probe the resolved base URL — inferred or configured — at startup, analogous to the ACP agent health probe, so a typo or an unbound port surfaces then rather than mid-turn. The probe runs on the gateway, so it confirms the *gateway* reaches the URL, not that the *sidecar* does; for a remote override the operator still owns correctness. For the inferred localhost case it is a useful sanity check that the gateway is actually listening on that port.
...
| No externally-reachable URL to configure | 🟢 | 🟡 inferred (`localhost`) for a co-located sidecar; `ai.mcp_base_url` only for remote | 🟢 |
