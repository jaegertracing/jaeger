...
| M7.2 | Decouple **UI tools** from `ai.enable_mcp`. The turn-scoped endpoint now mounts for every chat gateway to carry the per-turn UI tools; `enable_mcp` gates only whether the built-in telemetry tools are layered onto it (and the external shared endpoint). So `enable_mcp: false` with chat serves UI tools alone — UI-tool dispatch no longer depends on the telemetry gate, which M6 needs once it drops the ext-method path. | ✅ Done |
