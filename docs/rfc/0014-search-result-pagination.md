...
Mutual exclusivity rules out two things a reader might expect here. A zero `page_size` does not fall back to `search_depth`, because there is no second bound left to fall back to. And `search_depth` cannot double as an **overall traversal ceiling** on a paginated search, because a paginated request may not set it. Such a ceiling is still worth having — an unbounded "load more" should not be able to walk an entire index — but it needs a bound of its own rather than a second meaning for `search_depth` (§12).
...
2. **A default traversal ceiling.** Should the query service cap the number of pages a single cursor lineage yields by default, so an unbounded "load more" cannot walk an entire index, and if so at what bound? `search_depth` cannot serve as that ceiling now that it is mutually exclusive with `pagination` (§4), so the answer is a request field of its own, a server-side configuration limit, or that an unbounded traversal is acceptable and rate-limiting belongs to deployment.
