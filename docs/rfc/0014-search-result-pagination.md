...
- ✅ Reporting to the UI: the `searchPaginated` entry in the capabilities blob the UI reads, named after `searchWithoutServiceName` beside it. It is the conjunction of the reader's declaration and the `jaeger.query.pagination` gate, so the UI is never told to offer a continuation the query service would refuse. Delivered in [#9635](https://github.com/jaegertracing/jaeger/pull/9635).
- Still open: the token format of §3.2 with its query fingerprint and the check on continuation. It has nothing to verify until a reader mints a token, so it lands together with the first paginating backend (M3).
