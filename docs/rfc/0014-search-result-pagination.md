...
- ✅ The token of §3.1 and its binding of §3.2: the `pagetoken` package wraps a reader's cursor with the format version and the query fingerprint, and the query service checks both on continuation before handing the cursor back to the reader, so a backend only ever mints and reads its own cursor. Delivered in [#9652](https://github.com/jaegertracing/jaeger/pull/9652).
