...
- **M11 — Retire `olivere`.** Deletes `olivere` + the `go-elasticsearch/v9` template special-case (now unused, since every caller moved in Stage B). Sequenced as four independently reviewable sub-milestones so the removal lands last, once nothing imports `olivere` and the shutdown gap is closed. M11.2–M11.4 each depend on M11.1 (the owned client's `Close`); M11.4 additionally depends on M11.2 + M11.3 (no test consumers left). Delivered as self-contained PRs:
  ...
  - **M11.2 — Migrate the direct ES/OpenSearch integration tests off `olivere`. ✅ ([#8975](https://github.com/jaegertracing/jaeger/pull/8975)).** `internal/storage/integration` swaps the `olivere` `*elastic.Client` for a thin admin/inspection helper over the owned `esclient` (`Perform` for the handful of test-only REST calls — index create/delete, `_aliases`, settings, ILM/ISM, templates — that have no production method). *Exit:* the direct integration tests no longer import `olivere`; the ES 7–9 / OS 1–3 matrix stays green.
  - **M11.3 — Migrate the ES rotation e2e tests off `olivere`.** `cmd/jaeger/internal/integration` swaps `olivere` for the same esclient-backed admin helper; the ES-vs-OS branch (ILM vs ISM) is threaded from the caller (the tests already know their backend statically) rather than probed. *Exit:* the rotation e2e tests no longer import `olivere`; rotation smoke tests pass on both backends.
  - **M11.4 — Delete the `olivere` stack.** Migrates the factory `Purge` to `IndicesClient.DeleteAllIndices` and drops the legacy client; deletes the `es.Client` interface (`client.go`), its `ClientWrapper` (`wrapper/`), the `clientbuilder` package, `DetailedError` (`errors.go`), and the generated root `mocks/`; runs `go mod tidy` to drop `olivere`. `go-elasticsearch/v9` stays as a **transport-level** dependency (`esutil.BulkIndexer`, per M6); only its product-checked *client* special-case is gone. *Exit:* no `github.com/olivere/elastic` import (or `elastic.*` type) anywhere in the tree (§2.1 leak closed); full ES 7–9 / OS 1–3 matrix passes.
  - **M11.5 — Make `esclient.Client` a pointer handle.** `esclient.Client` is currently a value type (`NewClient` returns it by value; `IndicesClient`/`SearchClient`/`ILMClient`/`BulkIndexer` embed it by value; the factory and CLIs hold it by value). That is only copy-safe because its fields happen to be — a `*rawClient`, a duration, and a version enum — so the by-value contract silently leaks the requirement "stay copy-safe forever" to every embedder and caller, for negligible benefit (it is constructed once, never copied in a hot path). Convert it to a pointer-based handle (`NewClient` returns `*Client`; embedders/holders take `*Client`), the conventional shape for a resource that owns a connection pool. Pure refactor, no wire-format change. *Exit:* `esclient.Client` is used via pointer throughout; adding non-copy-safe state to it can no longer break callers silently.
