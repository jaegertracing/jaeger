...
- **M6 — Span writer (first write slice; bootstraps `BulkWriter` + bounded bulk indexer).** ✅ Done ([#8944](https://github.com/jaegertracing/jaeger/pull/8944)). Introduces `esclient.BulkWriter` + a bounded `BulkIndexer` (owned NDJSON encoder, hard byte ceiling that flushes *before* `FlushBytes` is exceeded, per-item 408/429/503/507 retry with backoff, preserved `bulk_index` metrics/logging) and migrates the span + service:operation write paths onto it over the shared transport pool. *Exit:* span-write snapshots byte-identical; hard byte cap (#2192) and per-item retry proven by test; write integration green.
