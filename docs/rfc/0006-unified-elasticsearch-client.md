...
**Stage C — Cleanup ✅ Done**
- **M11 — Retire `olivere`. ✅ Done ([#8968](https://github.com/jaegertracing/jaeger/pull/8968)).** Deletes the `olivere` data-plane client stack — the fat `es.Client` interface (`client.go`), its `ClientWrapper` (`wrapper/`), the `clientbuilder` package, `DetailedError` (its only `*elastic.Error` branch is dead), and the root `mocks/` — so the factory constructs only the owned `esclient`. Migrates the last `olivere` consumers, the ES/OpenSearch integration tests, onto a thin raw-HTTP test client. `esclient.Client` grows a `Close()` that releases the base transport's idle connections (`http.Transport.CloseIdleConnections`, forwarded through a `closeableRoundTripper`), wired into every ES storage factory's `Close()` — closing the one lifecycle gap left when the data plane stopped constructing the olivere client (which used to own that shutdown). `newRawClient` reinstates the clear "no servers specified" error olivere used to give. `go-elasticsearch/v9` **stays** as a transport-level dependency (the `esutil.BulkIndexer`, per M6 footnote); only its product-checked *client* special-case is gone. *Exit:* no `github.com/olivere/elastic` import (or `elastic.*` type) anywhere in the tree — §2.1 leak fully closed; `go mod tidy` drops `olivere` from `go.mod`; all ES snapshots byte-identical; full ES 7–9 / OS 1–3 matrix passes.
