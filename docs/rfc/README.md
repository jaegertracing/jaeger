...
- 📝 [0017: ClickHouse Schema — Comparison With the ClickStack / OTel Exporter Schema](./0017-clickhouse-schema-vs-clickstack.md) - Field-by-field comparison of Jaeger's ClickHouse schema with the OpenTelemetry `clickhouseexporter` schema that ClickStack documents, keeping the typed attribute layout while adopting its codecs, `LowCardinality`, table settings, and pairwise attribute skip indexes
