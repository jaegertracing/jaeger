...
- ³ Pairwise skip indexes remove granules that provably lack the pair; the surviving granules still pay a per-row scan (Jaeger's `arrayExists`, ClickStack's map lookup), so the result is bounded by selectivity rather than fixed.
...
- **M2 — Attribute skip indexes (§5.2).** Hashed `ALIAS` pair columns and `bloom_filter` indexes for the string, integer, and boolean groups at all five levels, the inline `has()` prefilter in the query builder, and the 0.001 default for the trace-ID filter on top of the option #8923 introduces. Measured by attribute-only search latency and `EXPLAIN indexes = 1`. Closes #8715; the configurable-rate half of #8918 is #8923's.
