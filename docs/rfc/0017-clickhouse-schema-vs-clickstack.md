...
| Key identity | Preserved: a key is an opaque string | Preserved | **Lost for dotted keys**: the `JSON` type reads `.` as nesting, so `{"http.status_code": 200}` and `{"http": {"status_code": 200}}` are the same path, and a map holding both fails insertion with `Duplicate path found`. Since nearly every OTel key is dotted, a Jaeger variant would have to escape dots on write and unescape on read |
...
- **M2 — JSON attribute spike (§5.3).** A benchmark-only branch storing attributes as `JSON` columns on the pinned ClickHouse, run against the §5.2 pair filter on the same 10M-span data so that the two are compared under one setup. Settles the two design questions and measures the pass criteria in §5.3. Its output is a decision, recorded in this RFC's status, and on a pass a superseding RFC.
