...
5. **Observability, not intervention.** The connector counts the spans it hands to the dead-letter pipeline (`jaeger_storage_writer_dead_letter_spans`) and logs each re-routed batch with a bounded sample of the backend's rejection reasons, so humans are *notified* asynchronously — but the pipeline has already recovered on its own. Optionally alarm on the counter's rate to catch a systemic mapping regression (a flood of terminal failures) versus the occasional bad document.
