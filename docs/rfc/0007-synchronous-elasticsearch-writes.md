...
The upstream contrib receiver is a fixed component — its dispatch (one goroutine per partition, serial within) and offset handling are not configurable — so this cannot be dialed in by config. Realizing it needs either a **custom Jaeger Kafka receiver** or an **upstream contribution** adding a worker pool + contiguous-offset commit. This supersedes the narrower "receiver-level batching" idea (M9): coalescing a single partition's fetch into one `ConsumeTraces` still binds parallelism to partition count; the worker-pool + serializer does not.
