...
- **M5 — Blocking batcher (config, plus one validation check). ✅ Done ([#9098](https://github.com/jaegertracing/jaeger/pull/9098)).** Adds the at-least-once ingester topology as an example config (`config-kafka-ingester-sync.yaml`): `queue.wait_for_result: true` + a byte-sized `queue.batch` on the storage exporter, no pipeline `batch` processor, and the Kafka receiver's `message_marking.after: true` / `on_error: false`. The `exporterhelper` fan-out/blocking is already proven by its own tests (§4.2). The one piece of new code: `jaeger_storage_exporter.start()` hard-fails when the storage writes synchronously (`write_mode: sync`) and the exporter's byte-sized `queue.batch.max_size` is unbounded or exceeds `bulk_processing.max_bytes` (§4.5) — an oversized batch would otherwise be split into several `_bulk` requests and lose per-batch atomicity. The exporter reads the write mode and byte cap through a small capability interface the ES factory implements, so it stays storage-agnostic and skips the check for backends that do not. *Exit:* the mismatched-`max_size` startup failure is covered by unit tests and the example config validates. The end-to-end proof that a failed batch advances no Kafka offset is delivered with the ingester validation (M7), which owns the kill-ES-mid-stream Kafka test.
...
- **M9 — (Optional, upstream) Receiver-level batching.** Open a contrib issue/PR to let the Kafka receiver coalesce per-partition records into one `ConsumeTraces` (§4.2 option 2). Independent of M1–M8; adopt if accepted.
