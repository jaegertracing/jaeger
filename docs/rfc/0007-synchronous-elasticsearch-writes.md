...
- **Status:** Implemented, except deferred M8 — graduated into [ADR-014](../adr/014-synchronous-elasticsearch-writes.md)
- **Author:** Yuri Shkuro
- **Created:** 2026-07-06
...
Async mode is the cheaper mode, not the safer one. Its cost is that a write failure is invisible upstream: the sender has already been told the spans were accepted, so it has no reason to retry them and no signal to slow down. An earlier version of this section argued that async is nevertheless the *right* mode for direct, unbatched ingest (OTLP receivers → storage, no Kafka) because the client owns retries there. That argument was dropped during implementation: a client that owns retries still needs the storage's verdict to know *when* to retry, and with async writes it never gets one. Sync mode closes the gap in both topologies. Behind the Kafka ingester it keeps the committed offset from running ahead of durable data. In front of OTLP clients it turns a storage outage into a retryable OTLP error (gRPC `Unavailable`, HTTP `503`), which is what lets an SDK or agent back off and retry from its own buffer.
...
- ✅ **M7 — Docs.** Done, in [jaegertracing/documentation](https://github.com/jaegertracing/documentation) (the Elasticsearch page's *Write Modes* section and the Kafka page's *At-least-once delivery* section, both in the `_dev` tree for the next release). Document `write_mode` **and** the co-required settings (`message_marking.after: true`, blocking batcher, no `batch` processor — §4.5), the deterministic-`_id`/idempotency behavior (§4.7), the dead-letter pipeline and how to attach a sink (§4.8), upstream-sizing guidance, the durability-vs-searchability note (§2.2), and the acking rationale (§4.6). *Exit:* configuration guide updated.
