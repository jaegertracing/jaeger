...
Synchronous writes earn their cost specifically where there **is** a durable buffer to be honest with — the **Kafka ingester** — and where losing an ack means losing data that Kafka still believes was delivered. This is why the two modes coexist: `async` for unbatched direct ingest, `sync` for Kafka-buffered ingest. Whether async is ever fully removed (vs. retained as a non-default option for the direct-ingest path), and how the default migrates to sync, are left open (§7).
...
¹ franz-go's own default is 50 MiB; the receiver overrides it to 1 MiB. Read-ahead is additionally bounded by `MaxBufferedRecords` (franz-go default 10000) and, once buffers fill, back-pressure stops fetching until the next poll.

So each fetch already arrives as a byte-bounded *bulk* of records. The ceiling is downstream of the fetch: the receiver **re-atomizes** that bulk into one `ConsumeTraces` per message, dispatched with per-partition serialism, so the storage batch can still only coalesce roughly as many as there are partitions.
...
**Desired consumer behavior for the synchronous batched path.** Combining the franz-go receiver's fetch/marking machinery with the v1 learnings, the target is: (1) poll a large fetch (as franz already does); (2) dispatch records to a **worker pool whose size is independent of partition count**, whose members are the producers into the blocking byte-batcher (§4.2 option 1), so a batch fills to `max_bytes` regardless of how many partitions this replica owns; (3) write each full batch as **one synchronous `_bulk`**, and on success hand the batch's completed offsets to a **per-partition contiguous-offset serializer** that commits the highest contiguous offset — on failure, do not advance and retry the batch (idempotent via §4.7); (4) detect and break stalls, and dispose of poison documents autonomously (§4.8) so no partition head-of-line-blocks.

The upstream contrib receiver is a fixed component — its dispatch (one goroutine per partition, serial within) and offset handling are not configurable — so this cannot be dialed in by config. Realizing it needs either a **custom Jaeger Kafka receiver** or an **upstream contribution** adding a worker pool + contiguous-offset commit. This supersedes the narrower "receiver-level batching" idea (M8): coalescing a single partition's fetch into one `ConsumeTraces` still binds parallelism to partition count; the worker-pool + serializer does not.
...
3. **Terminal → dispose, then advance.** The disposition is operator-selectable (`poison_pill_handling`), because tolerance for losing a malformed span differs:
   ...
   - `dead_letter` — re-emit each terminal item onto a **separate OTel dead-letter pipeline** rather than a sink Jaeger implements. This disposition is selected by **topology**, not by a config value: the storage write is placed in the pipeline as the `jaeger_storage_writer` traces→traces **connector** instead of `jaeger_storage_exporter`, and the connector's output is wired to a dead-letter pipeline ending in any standard exporter — a Kafka exporter for a Kafka DLQ, `file`, `debug`/logging, or OTLP — so the sink is the operator's choice with **no dead-letter sink code in Jaeger**. The storage stays in `fail` mode: the writer's job is to *report* the rejection, and what a rejection means is the caller's decision — the exporter holds the offset, the connector dead-letters. A third `poison_pill_handling: dead_letter` value was considered and dropped, because it would state the same decision in two places (the enum and the pipeline wiring) that could only ever be validated for agreement.
...
5. **Observability, not intervention.** The connector counts the spans it hands to the dead-letter pipeline (`jaeger_storage_writer_dead_lettered_spans`) and logs each dead-lettered batch with a bounded sample of the backend's rejection reasons, so humans are *notified* asynchronously — but the pipeline has already recovered on its own. Optionally alarm on the counter's rate to catch a systemic mapping regression (a flood of terminal failures) versus the occasional bad document.
...
**Component shape.** Only a connector can re-emit spans: a terminal exporter has no downstream consumer. The concern that a connector forgoes `exporterhelper`'s sending queue — the blocking batcher and retry that couple the offset to durability (§4.2) — is resolved by construction: `jaeger_storage_writer` builds the same `exporterhelper` traces pipeline `jaeger_storage_exporter` is built on, with the connector's `ConsumeTraces` feeding it and the connector's write-and-tap function as its push step. It therefore takes the exporter's `queue` (including `wait_for_result` and `batch`) and `retry_on_failure` settings unchanged, and the ingester shape of §4.5 carries over with the exporter swapped for the connector. Per-item detail reaches the connector without widening any writer interface: in `fail` mode the synchronous writer returns a typed `*esclient.BulkWriteError` listing every terminally-rejected document (`_id`, index, status, reason) and whether transient failures also occurred, aggregated across `_bulk` chunks and surviving `errors.Join`, so the connector recovers it with `errors.As`. It maps each rejected span `_id` back to its source span through the `traceID_spanID` prefix (§4.7), copies exactly those spans (with their resource and scope) onto its output, and returns success only once the dead-letter pipeline has accepted them. A batch that also had transient failures is returned as an error *without* dead-lettering, so that the retry — which re-sends the poison too — dead-letters each span once, when the batch is otherwise complete. A rejected document that names no span (a service/operation lookup document) is logged and discarded; its spans are durable and any later span of that service re-emits it. The connector refuses to start against a storage that does not report rejections (`tracestore.PoisonPillReporting`: for ES/OS, `write_mode: sync` with `poison_pill_handling: fail`), because it would otherwise degrade silently into the exporter with a dead-letter pipeline that never receives a span.
