...
- **Per-item outcomes are delivered through callbacks, not a return value.** Each item's result is reported to the `OnSuccess`/`OnFailure` closures passed on `Add`; `Flush` "returns an error only for transport-level failures or context cancellation." A `Flush` that returns `nil` therefore does **not** mean every document was indexed — a poison document (e.g. a mapping rejection) is a perfectly successful HTTP `200` at the transport level and surfaces *only* through that item's `OnFailure`. But a conformant `WriteTraces` must return *"error iff any item was rejected"* (§1.1, §4.8), which `Flush`'s return value cannot express.
