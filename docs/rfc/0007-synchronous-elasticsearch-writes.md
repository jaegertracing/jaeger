...
The component shape is a traces→traces **connector** (`jaeger_storage_dead_letter`), not an exporter-with-tap: a connector has a downstream consumer, which a terminal `jaeger_storage_exporter` structurally lacks, so it is the only shape that can re-emit the rejected spans onto another pipeline. The connector resolves the trace writer from the `jaeger_storage` extension exactly as the exporter does and writes each batch via `WriteTraces`. Per-item poison detail reaches it without widening any interface (`WriteTraces`/`WriteSpans`/`WriteBatch` still return a bare `error`, §4.1): in `fail` mode the synchronous writer returns a typed `*esclient.BulkWriteError` carrying every terminally-rejected item (its `_id`, index, status, reason) and a flag for whether transient failures also occurred, aggregated across `_bulk` chunks into one error that survives `errors.Join` and pass-through so the connector recovers it with `errors.As`. The connector maps each poison item's deterministic `_id` back to its source span by its `traceID_spanID` prefix (§4.7), filters the original `ptrace.Traces` to exactly those spans, and emits them to its output pipeline (any standard exporter — Kafka, `file`, `debug`, OTLP). It returns success — the offset advances — when every failure was terminal and the dead-letter sink accepted the spans, and returns the error — holding the offset for retry — when transient failures remain or the dead-letter sink itself rejects the spans (§4.8 step 4). `dead_letter` is thus expressed **structurally**, by wiring the connector's output to a dead-letter pipeline, rather than as a third `poison_pill_handling` value: the storage-level enum stays `fail | drop`, and the connector is used against a backend in `fail` mode.
...
- **M5 — Poison-pill handling — §4.8.** Classify per-item `_bulk` failures into transient (retry the batch) vs. terminal (poison), and let the operator choose the terminal disposition: `fail` (default — retry forever, never drop), `drop` (discard the poison doc so the offset advances), or `dead_letter` (re-emit it onto a separate OTel pipeline). **Delivered so far:** the classification and the `poison_pill_handling: fail|drop` config for the synchronous writer — `drop` is the "some operators are fine losing a rare malformed span" option and needs no pipeline. **Prototyped ([#9147](https://github.com/jaegertracing/jaeger/pull/9147)): `dead_letter` — the `jaeger_storage_dead_letter` traces→traces connector.** The synchronous writer surfaces terminal rejections through a typed `*esclient.BulkWriteError` (each poison item's `_id`, index, status, reason, plus a transient-failures flag) that survives `errors.Join`/pass-through, and the connector recovers it with `errors.As`, maps each poison `_id`'s `traceID_spanID` prefix back to its source span, and emits only those spans to a dead-letter pipeline (any standard exporter — Kafka, `file`, `debug`, OTLP) with **no custom sink code in Jaeger**. It advances the offset when all failures were terminal-and-dead-lettered and holds it (returns the error) when transient failures remain or the sink rejects. This settles the open component-shape question in favor of a connector (an exporter has no downstream consumer to tap onto); the mechanism is described in §4.8. **Remaining:** the integration test injecting a permanently-rejected document to show, under `dead_letter`, the poison span emitted to the dead-letter pipeline end-to-end. Poison handling is what makes `write_mode: sync` production-safe: no head-of-line blocking and no silent loss. Required before the default flips to `sync`. *Exit:* an integration test injecting a permanently-rejected document (e.g. a mapping conflict) shows, under `drop`, the poison discarded and the batch completing so the offset advances (partition never stalls) and, under `dead_letter`, the poison span emitted to the dead-letter pipeline; a transient failure still holds the offset.
- **M6 — Ingester at-least-once: blocking batcher + end-to-end validation.** Provide the ingester pipeline shape that couples the Kafka offset to durability — `queue.wait_for_result: true` + `queue.batch` on the storage exporter, the pipeline `batch` processor removed, the Kafka receiver's `message_marking.after: true` — as an example config exercised in CI (the `exporterhelper` fan-out/blocking itself is already proven by its own tests, §4.2). **Delivered so far:** the example config (`config-kafka-ingester-sync.yaml` — ES backend, `write_mode: sync`, `poison_pill_handling: drop`, blocking batcher) and a CI e2e (`TestKafkaStorage_SyncElasticsearch`) driving Collector → Kafka → Ingester → Elasticsearch through the synchronous write path; ES is started alongside Kafka in `scripts/e2e/kafka.sh`. Safe to run now because `drop` (M5) prevents a poison span from stalling the partition, making sync+drop behaviorally close to async. **Remaining:** the fault-injection assertion — kill/reject ES mid-stream and assert no offset advance, full recovery on ES return with **no duplicates** (idempotent `_id`) and **no stall**. *Exit:* at-least-once demonstrated end-to-end, autonomously; a failed batch advances no offset. (A standalone byte-sizing warning — local exporter hardening, not this milestone — shipped in [#9098](https://github.com/jaegertracing/jaeger/pull/9098).)
