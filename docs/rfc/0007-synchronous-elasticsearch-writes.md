...
- **M4 — Wire sync path + `write_mode` config.** Introduce the `write_mode: async|sync` config (default `async`; explicit wins; unrecognized rejected by whole-config validation) **together with** the code that honors it — not as a standalone knob (see the guiding constraint above). Wire `TraceWriter.WriteTraces` → `WriteSpans` → synchronous bulk when `write_mode: sync`; append service/operation docs into the same request; update the service cache only after success (§4.3); reuse `max_bytes` as the safety-net chunk cap. *Exit:* config parse/validate/default tests; the full ES/OS trace-storage integration suite passes with `write_mode: sync` (parametrized alongside the async run) across the ES 7–9 / OS 1–3 matrix; plus a fault-injection integration test asserting a failing bulk makes `WriteTraces` return an error (and, on the ingester, the Kafka offset is **not** committed).
