...
The contract in §6 is defined on the body, not on the transport coding. The gateway writes each line to the `http.ResponseWriter` and flushes through `http.NewResponseController(w).Flush()`. Flushing is best-effort as far as error reporting goes: the response controller prefers a writer's void `http.Flusher.Flush` over the error-returning path, and every wrapper in the chain (otelhttp, the recovery handler, `CompressHandler`) exposes the void form, so a failed flush is not observable to the handler. Disconnects are therefore detected through the request context and through `Write` errors, as §6.4 states, and never through `Flush`. A writer with no flush support at all would make streaming meaningless, so the gateway checks for it once, before committing the status, and falls back to the buffered response. The standard library chooses the framing:
