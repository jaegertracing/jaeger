...
The contract in §6 is defined on the body, not on the transport coding. The gateway writes each line to the `http.ResponseWriter` and flushes through `http.NewResponseController(w).Flush()`, which sees through the wrapping writers in the chain via `Unwrap` and, unlike `http.Flusher.Flush`, returns an error; a writer that cannot flush is a configuration error that the gateway reports once and then treats as a failed write (§6.4). The standard library chooses the framing:
