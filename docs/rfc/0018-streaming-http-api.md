...
- A client that sends no `Accept`, `*/*`, or `application/json` receives the same status and body it receives today (the headers gain `Vary: Accept` and, when negotiated, `Content-Encoding` and `Vary: Accept-Encoding`), with one exception: an empty `FindTraces` search returns `200` with `{"result":{}}` instead of `404` (§6.6).
...
- **M2 — Response compression on the query HTTP server (jaeger).** `handlers.CompressHandler` behind a gzip-only `Accept-Encoding` adapter (§8 ⁴) in the `cmd/jaeger/internal/extension/jaegerquery/internal/server.go` chain; a test that streams through gzip and shows the client decoding each line before the stream ends; tests that `Accept-Encoding: deflate, gzip` and `gzip;q=0.8` yield gzip while `deflate` and `gzip;q=0` yield identity; a check that the SSE chat and MCP endpoints still stream through the compressor and that the OTLP reverse proxy still forwards the client's `Accept-Encoding`.
