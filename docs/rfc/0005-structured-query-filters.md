...
3. **Check the constants.** A constant compared against a built-in field is parsed as that field's declared type and refused when malformed (§5.2). This is what stops the same query being rejected on a backend that receives the legacy predicate fields and accepted by one that evaluates the filter natively.
...
- **A semantic type system for the AST** — a type algebra (`Bool`, `Duration`, `Timestamp`, `TraceID`, `Dynamic`, …), an operator signature table, inferred result types on every `Call`, and a pass returning a tree annotated with them. *Deferred, not rejected*, and §6.1's forward-compatibility rules are what keep deferring it from foreclosing it: the Go AST types are public API the moment jaeger-idl releases, so a decision to add semantic annotations later has to still be additive then. The part that pays for itself now is in stage 3 above: built-in fields declare their types, so a malformed constant is refused once instead of differently per backend. The rest waits on a second backend that evaluates filters natively, because designing coercion rules against a single implementation encodes that implementation's assumptions as though they were the API's — which is the mistake this RFC already made twice, with a `boolean` capability flag and with normalization toward the filter. Two parts of the proposal look speculative rather than early: a result type on `Call` matters only once a call may appear as a comparison operand, which no operator does today, and distinguishing "no constraint on this literal" from "this attribute's type is storage's to decide" is a distinction with no consumer while backends resolve attribute types themselves. What would trigger revisiting: a second native backend, or a coercion question that cannot be answered per-backend.
