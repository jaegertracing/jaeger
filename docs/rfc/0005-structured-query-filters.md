...
| `eq`, `ne` | a reference and a constant | the two values are equal, compared as the constant's type |
| `gt`, `lt`, `gte`, `lte` | a reference and a constant | ordered comparison, within one domain (below) |
...
**A comparison compares one reference against one constant.** Either order is accepted, since a caller writing JSON by hand may put the constant first, and the query service turns the comparison around — swapping the operands and reversing an ordered operator — so that everything downstream reads one shape (§7). Two constants ask nothing about the span. Two references are refused: no backend can evaluate one, and what comparing a duration against a name should mean is a question this version does not answer. Allowing it later is additive, and would arrive with the compatibility rules for each pair of field types.
