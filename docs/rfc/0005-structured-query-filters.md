...
| **Elasticsearch / OpenSearch** | Denormalized object fields (`tag.*`, `process.tag.*`) + nested arrays (`tags`, `process.tags`, `logs.fields`) | Partial — span / resource / log are distinct; scope identity is stored in span tags and link IDs in references | Scope name/version and link IDs are searchable; scope and link attributes need schema evolution |
...
- **Storage-schema changes.** The model is designed to fit existing schemas; where a backend's schema cannot express a level (ES scope/link attributes, flat-index link), that is a documented limitation, not a schema migration mandated by this RFC.
...
On Elasticsearch/OpenSearch, `span.startTime` uses the existing `startTimeMillis` date field and the same epoch-millisecond conversion as the query time bounds. Its constants are truncated to milliseconds; this filter does not preserve the microsecond precision of the separate `startTime` field. `event.time` uses `logs.timestamp`, which stores epoch microseconds. Neither filter changes the storage schema.

Link identity needs one storage-specific qualification. Elasticsearch/OpenSearch stores references as a nested array, but older Jaeger writers also put the span's parent there as a synthetic `CHILD_OF` reference. Link-ID predicates first use the indexed nested field to select candidates, then inspect the stored source document to exclude only a same-trace, same-span synthetic parent; a `CHILD_OF` link to a different trace remains a link. This requires `_source` and server-side scripts. A broad link query therefore costs source decoding and a reference walk for each indexed candidate, and is refused if that execution environment cannot run it rather than returning a parent as a link. A genuine `CHILD_OF` link to exactly the parent may already have been coalesced with the synthetic reference by an older writer, so existing data cannot distinguish it from a span with no link. No reindex of that stored representation can restore the discarded provenance. Pattern matching on link IDs is refused on this backend: the indexed engine and the source-script engine do not share the RFC's RE2 semantics, and accepting an approximate pattern would make the parent exclusion unsound.
...
- **Levels** — ClickHouse honors all five; ES/OS honor span/resource/event and the indexed scope/link fields (scope and link attributes await schema evolution); the flat backends honor only span/resource/event (§1.6). The honored level set is declared, so a predicate naming an unsupported level is refused, not widened.
...
- ✅ **M4 — Elasticsearch/OpenSearch.** Route span/resource/event levels to their fields and lower the boolean tree into a `bool` query; scope and link identity fields are also supported, while their attributes remain refused. **Delivered in [#9350](https://github.com/jaegertracing/jaeger/pull/9350), which lowers the filter in [`core/filter.go`](../../internal/storage/v2/elasticsearch/tracestore/core/filter.go) and `buildFilterQuery` ([`core/reader.go`](../../internal/storage/v2/elasticsearch/tracestore/core/reader.go)), and declares what the reader can serve through `FilterCapabilities()`: the span, resource, event, scope and link levels, `and`/`or`/`not`, and `eq`, `ne`, `gt`, `lt`, `gte`, `lte`, `regex`, `exists`, `in` and `not_in`. Scope and link attributes, which this schema does not store, and the `some` quantifier, whose correlated matching over a span's events is not implemented, are refused rather than approximated. Ordering reached only `span.duration` at first, the one field this schema maps as a number: every attribute is a `keyword`, where a range compares lexicographically, so `gt`/`lt`/`gte`/`lte` on one was refused rather than answered wrongly. [RFC 0015](./0015-typed-attribute-indexing-elasticsearch.md) lifted that in [#9321](https://github.com/jaegertracing/jaeger/pull/9321), which indexes an attribute value as a number beside the keyword and ranges over that sub-field; it is behind an alpha feature gate, so with the gate off the refusal is still what a caller gets. Either way the declaration stays coarser than the reader — the operator is declared and the level is declared, and only the pairing can be unservable, so this refusal comes from the reader at query time rather than from the capability check.** *Exit:* level-qualified and boolean queries lower to the corresponding `bool` query; the pre-existing unqualified snapshots are unchanged. **What the reader declares is now checked against a running cluster** by the filter battery in the shared storage integration suite ([`filters.go`](../../internal/storage/integration/filters.go), [#9381](https://github.com/jaegertracing/jaeger/pull/9381)): every declared level, built-in field and operator searches a four-trace corpus and is held to an exact result set, and the two shapes the reader refuses — scope attributes and `some` — are asserted to come back as refusals rather than as a wider answer. It runs against Elasticsearch and OpenSearch in both tag-storage modes, and both through a Reader directly and through the api_v3 edge of a running jaeger binary.
...
- ES/OS schema evolution to index scope and link attributes distinctly (§1.6) — extends those levels beyond their indexed identity fields in M4.
