...
| **Elasticsearch / OpenSearch** | Denormalized object fields (`tag.*`, `process.tag.*`) + nested arrays (`tags`, `process.tags`, `logs.fields`) | Partial — span / resource / log are distinct; the scope and the link keep their identity but not their attributes | Span/resource/event levels work; a scope's name and version and a link's IDs are indexed, and the attributes of those two levels need schema evolution |
...
- **Storage-schema changes.** The model is designed to fit existing schemas; where a backend's schema cannot express a level (the scope's and the link's attributes on ES, the link on a flat index), that is a documented limitation, not a schema migration mandated by this RFC.
...
- **Levels** — ClickHouse honors all five; so do ES/OS, though the scope and the link are honored only for the fields naming their identity, their attributes awaiting schema evolution; the flat backends honor only span/resource/event (§1.6). The honored level set is declared, so a predicate naming an unsupported level is refused, not widened. A level a backend serves in part is declared and the unservable predicate inside it refused by the reader, since a declaration names levels and operators and cannot tell a built-in field from an attribute key.
...
- ✅ **M4 — Elasticsearch/OpenSearch.** Route each level's predicates to the fields holding them and lower the boolean tree into a `bool` query; what the schema does not index — an instrumentation scope's and a link's attributes — is rejected pending schema evolution. **Delivered in [#9350](https://github.com/jaegertracing/jaeger/pull/9350), which lowers the filter in [`core/filter.go`](../../internal/storage/v2/elasticsearch/tracestore/core/filter.go) and `buildFilterQuery` ([`core/reader.go`](../../internal/storage/v2/elasticsearch/tracestore/core/reader.go)), and declares what the reader can serve through `FilterCapabilities()`: the span, resource and event levels, `and`/`or`/`not`, and `eq`, `ne`, `gt`, `lt`, `gte`, `lte`, `regex`, `exists`, `in` and `not_in`. The scope and link levels, which this schema does not index distinctly, and the `some` quantifier, whose correlated matching over a span's events is not implemented, are refused rather than approximated. Ordering reaches only `span.duration`, the one field this schema maps as a number: every attribute is a `keyword`, where a range compares lexicographically, so `gt`/`lt`/`gte`/`lte` on one is refused rather than answered wrongly. [RFC 0015](./0015-typed-attribute-indexing-elasticsearch.md) is what lifts that, and until it does the declaration is coarser than the reader — the operator is declared and the level is declared, and only the pairing is unservable, so this refusal comes from the reader at query time rather than from the capability check.** *Exit:* level-qualified and boolean queries lower to the corresponding `bool` query; the pre-existing unqualified snapshots are unchanged. **What the reader declares is now checked against a running cluster** by the filter battery in the shared storage integration suite ([`filters.go`](../../internal/storage/integration/filters.go), [#9381](https://github.com/jaegertracing/jaeger/pull/9381)): every declared level, built-in field and operator searches a four-trace corpus and is held to an exact result set, and the shapes the reader refuses are asserted to come back as refusals rather than as a wider answer. It runs against Elasticsearch and OpenSearch in both tag-storage modes, and both through a Reader directly and through the api_v3 edge of a running jaeger binary. **The refusals were then narrowed from the level to the field in [#9474](https://github.com/jaegertracing/jaeger/issues/9474):** a level whose identity is indexed and whose attributes are not is the common case rather than the exception here, so every level is declared and `fieldColumns` says which built-in fields have somewhere to go. `span.traceID` and `span.spanID` are the span document's own keywords, `scope.name` and `scope.version` the span tags the write path folds the instrumentation scope into (`getTagsFromInstrumentationLibrary`), and `link.traceID` and `link.spanID` the keywords inside the `references` nested type. `span.startTime` and `event.time` lower to a range over `startTime` and `logs.timestamp`, the microsecond columns the write path fills, which is what makes the timestamp constant type searchable; a scope's and a link's attributes, which nothing writes, stay refused, and so does a built-in field no column holds — `link.traceState`, `span.endTime` — each named in the refusal.
