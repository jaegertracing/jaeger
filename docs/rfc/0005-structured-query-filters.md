...
1. **Same-span enforcement on the flat backends.** `filter` conjunctions are same-span by design (§7), inheriting the semantics api_v3 documents for `attributes`. What is open is whether every backend can honor it: a flat inverted index (Cassandra/Badger) can intersect tag matches at *trace* rather than *span* granularity, returning a trace where different spans satisfy different predicates. Is same-span a conformance requirement every backend must meet, or a capability a backend declares it cannot (as it does for levels and operators, §7)?
