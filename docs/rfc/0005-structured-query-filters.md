...
| `regex` | a reference and a string | the value read as text matches the expression |
...
**IDs, status and span kind stay strings for now.** They are string spellings with a validated vocabulary, and a distinct node type only pays once a consumer wants the parsed form. `TraceID` in particular would force a representation decision — bytes or hex — into a package that should not take a dependency on a trace-data library to express a query. They are validated against their spelling, not given types.
