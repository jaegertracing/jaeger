...
**Milestone 5 — gRPC remote storage.** A `Capabilities` RPC on `jaeger.storage.v2`'s `TraceReader` in `jaeger-idl`, so a remote backend reports for itself: the server side answers from the reader it fronts, and the client maps `Unimplemented` to `ErrUnsupported`, which is how the same reader already handles a backend without `FindTraceSummaries`. §3.6 also floated an operator-set config field as a near-term stopgap; it is not being built, because it would add a config surface to deprecate once the RPC exists, and because it would not help CI — the end-to-end suites cannot set it, so they would stay excused from the cross-service test that the RPC lets them run.
