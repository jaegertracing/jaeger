...
**Milestone 5 — gRPC remote storage.** ✅ [#9264](https://github.com/jaegertracing/jaeger/pull/9264) A `Capabilities` service on `jaeger.storage.v2` in `jaeger-idl` ([jaeger-idl#211](https://github.com/jaegertracing/jaeger-idl/pull/211)) — a separate service rather than a method on `TraceReader`, so a backend can serve one without the other — so a remote backend reports for itself: the server side answers from the reader it fronts, and the client maps `Unimplemented` to `ErrUnsupported`, which is how the same reader already handles a backend without `FindTraceSummaries`. Nothing holds a copy of the answer: the query service asks the reader on every service-less search, and the static handler asks it on every SPA serve to build the capability blob — the same per-serve treatment the AI-assistant flag already gets. That is because jaeger-query can come up before its remote backend is reachable, and any value captured at startup would pin the process to the least capable behaviour for its whole life, including in the UI. Caching lives in the one reader whose answer costs a round trip: the gRPC reader remembers what its backend said, never a failure, since the proto declares capabilities stable for the lifetime of a connection. Every other reader answers from a constant. This is what lets the remote-storage integration suite and the `grpc` and `query` end-to-end suites run the cross-service search instead of opting out of it — in the `query` topology jaeger-query starts before its remote backend, so a startup-only read would have left it refusing the search the suite now runs. §3.6 also floated an operator-set config field as a near-term stopgap; it was not built, because it would add a config surface to deprecate once the RPC existed, and because it would not have helped CI — the end-to-end suites cannot set it, so they would have stayed excused from the cross-service test the RPC lets them run.
